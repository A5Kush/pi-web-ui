...
│   ├── tool-manager.ts         # ★ Agent 工具统一开关：AGENT_TOOL_CATALOG（终端 7＋子代理 7＋其他 5：edit_soft/delegate_task/ask_user_question/todo_list/browser_page）＋ tool_manage 出入口（setAgentToolEnabled/applyAgentToolsGating），持久化只有 disabledAgentTools，遗留三开关双向同步
...
│   │   ├── ui-slots.ts         # ★ 宿主 UI 扩展点引擎（issue #146 完整版）：BUILTIN_UI_ITEMS（32 条宿主内置入口）+ buildUiSlots（宿主默认→插件贡献→插件 arrange→用户偏好 四层合并，11 个 slot）+ splitOverflow/restoreUiItem
...
│   │                           #     context-menu-ui-test（右键菜单槽位）/ ui-layout-ui-test（布局页 ↔ 界面一致性）/ plugin-settings-page-test（插件设置页）/ plugin-topbar-ui-test（顶栏条目 + 面板内后台卸载）
...
| **插件**             | `docs/architecture-plugins.md`                         | <dataDir>/plugins/<id>/ 目录（manifest.json + index.mjs + client/entry.mjs）；attach 时热重扫；fenced-code 渲染插件（renderers + view:false，命中 ```lang 才懒加载，见 plugin-fence.ts）；官方插件走 `pi-web-ui install`（含子目录 source）分发，不进 npm 包；插件市场（plugins/catalog.json 内置列表 + 用户自定义，设置面板一键 `install --name <id>`，见 plugin-catalog.ts）；插件后台作业（安装/更新/卸载不占用户终端、设置面板就地显示进度，见 server/plugin-installer.ts）；`host.reloadCatalog()` 市场目录同步（#148）；`pi-web-ui install --build` 源码隔离构建（#150）；**UI 扩展点框架**（#146 完整版）：manifest `ui` / `host.ui.*` 贡献 + `arrange` 整理，宿主负责渲染/排序/溢出/可访问性/用户偏好/审计，插件不碰 DOM（动作经 `host.onUiAction` 回插件），合并规则与 11 个挂载点见 `web/src/ui-slots.ts`；插件自定义设置页（`settings.pages` → PluginPage.tsx）；通用右键菜单（contextmenu.* → ContextMenu.tsx）；目录授权与跨目录 fs（`server/plugin-grants.ts` + `host.fs.requestAccess/authorizedDirs/listPath/readPath/readTextPath/writePath/removePath`，工作区内免授权）；项目组装 `host.project.create`（`server/plugin-project.ts`）；插件→宿主动作桥 `window.__piWebUiHost`（宿主 API **v7**：setView / startChat / compose（塞输入框草稿，见 web/src/composer-bridge.ts）/ openSession（带目录授权，多根）/ sessions.list\|open / reloadCatalog / onUiAction / pageCall / dom.anchors（特权 DOM 挂载点，见下），时序见 web/src/plugin-host.ts）；特权 DOM：manifest `permissions` 含 `dom` 族的插件 bundle 默认 403，需用户在设置面板逐个授权（`server/plugin-dom.ts` 的授权表 + `plugin_dom_consent`，授权后 epoch+1 重拉），限制的是“谁可以”而非能力；宿主设施版本 `PLUGIN_API_VERSION = 2`（manifest `apiVersion` 高于宿主则拒绝激活）；MCP 工具桥 |
...
- **新增「可隐藏 / 可排序」的宿主入口必须在渲染组件里接上 slot**：布局页的勾选框与 ↑↓ 只对**真按 slot 列表渲染**的东西生效。顶栏「桌面工具组」与底栏整条由 `TopBar.tsx` / `FooterBar.tsx` 里的 `hostNodes` 节点工厂 + `uiPrimary`/`bottombarItems` 顺序驱动（顶栏隐藏的入口落进「⋯」溢出菜单：菜单型整块搬组件、动作类走 `dispatchHostOverflow`），消息工具条由 `Message.tsx` 跳 `hidden`，右栏 tab 由 `RightPanel.tsx` 按 `uiRightPanelTabs` 顺序渲染。**加新入口忘了这一处 = 一个点了没反应的勾选框**（历史上就这么退化过）。另外：slot 数据没传时要退化成「按内置默认顺序全画」，别把整条顶栏/底栏清空。回归：`tests/ui-layout-ui-test.mjs`（真 Chrome，43 checks）。
