...
| `PI_WEB_PLUGIN_CATALOG_URL`    | official catalog URL              | Boot-time catalog pre-sync: fetch a catalog document (http(s) URL or absolute path), write the installable list, then install/update every entry (failures only warn). Defaults to the official community registry `https://xing-shuyin.github.io/pi-web-ui-plugins/catalog.json`; set to empty or `off`/`0`/`false`/`no` to disable. |
