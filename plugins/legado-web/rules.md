...
5. `legado_source_probe` 把修好的链路再跑一遍确认；通过后如果该源曾被标成废源/可疑
   （`legado_book_sources` 的 `list` 里能看到健康状态），用 `action: "unmark"` 把标记摘掉——
   **否则页面默认隐藏废源，用户会以为源丢了**。摘完标记页面会自动重读出现。
6. 告诉用户刷新阅读页（浏览器里那份是内存副本，需要重新从后端 hydrate）；写完插件也会弹一条通知。
